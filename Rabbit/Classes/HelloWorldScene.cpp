...
bool HelloWorld::init()
{
	...
	//get bunny
	m_pNode_rabbit = pScene->getChildByName("Sprite_Rabbit");
	m_pNode_rabbit->removeFromParent();
	m_pNodes->addChild(m_pNode_rabbit);
	...
	//get stones
	{
		...
		for (int i = -1; i <= 7; i++)
			for (int j = 1; j <= 9; j++) {
				...
				cocos2d::Node* pnode = pScene->getChildByName(ostr.str());
				pnode->removeFromParent();
				m_pNodes->addChild(pnode);
				m_pNode_stones.pushBack(pnode);
			}
	}

	//get text node
	{
		cocos2d::Node* p = pScene->getChildByName("Text_curStep");
		p->removeFromParent();
		m_pNodes->addChild(p);

		p = pScene->getChildByName("Text_bestStep");
		p->removeFromParent();
		m_pNodes->addChild(p);
	}
    ...
    //获取UITEXT
    m_pText_curStep = dynamic_cast<Text* >(m_pNodes->getChildByName("Text_curStep"));
    m_pText_bestStep =  dynamic_cast<Text* >(m_pNodes->getChildByName("Text_bestStep"));
...
void HelloWorld::setRandStones(int num){
    ...
    while (num!=curNum) {
     int ver,hor,tag;
        ...
        tag = ver*NUM_MAPROWANDCOW+hor + 3;	//start at 4
		cocos2d::Node* p = m_pNodes->getChildByTag(tag);
		if (p) {
